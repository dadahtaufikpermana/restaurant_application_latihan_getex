part of 'app_pages.dart';
// DO NOT EDIT. This is code generated via package:get_cli/get_cli.dart

abstract class Routes {
  Routes._();
  static const HOME = _Paths.HOME;
  static const SPLASH = _Paths.SPLASH;
  static const MENU_FOOD = _Paths.MENU_FOOD;
  static const LOGIN = _Paths.LOGIN;
  static const HOME_SCREEN = _Paths.HOME_SCREEN;
  static const HOME_VIEW = _Paths.HOME_VIEW;
  static const HISTORY = _Paths.HISTORY;
  static const NETWORK = _Paths.NETWORK;
  static const NO_OFFER = _Paths.NO_OFFER;
  static const SIGN_UP =  _Paths.SIGN_UP;
  static const TAB_CONTROLER =  _Paths.TAB_CONTROLER;
  static const CART = _Paths.CART;
  static const CHECKOUT = _Paths.CHECKOUT;
  static const PAYMENT = _Paths.PAYMENT;
  static const PROFILE = _Paths.PROFILE;
  static const PROFILE_CHANGE = _Paths.PROFILE_CHANGE;
  static const ORDER = _Paths.ORDER;
}

abstract class Splash {
  Splash._();
  static const SPLASH = _Splash.SPLASH;
}

abstract class _Paths {
  _Paths._();
  static const HOME = '/home';
  static const SPLASH = '/splash';
  static const MENU_FOOD = '/menu-food';
  static const LOGIN = '/login';
  static const HOME_SCREEN = '/home-screen';
  static const HOME_VIEW = '/home-view';
  static const HISTORY = '/history';
  static const NETWORK = '/network';
  static const NO_OFFER = '/no-offer';
  static const SIGN_UP = '/sign-up';
  static const TAB_CONTROLER = '/tab-controler';
  static const CART = '/cart';
  static const CHECKOUT = '/checkout';
  static const PAYMENT = '/payment';
  static const PROFILE = '/profile';
  static const PROFILE_CHANGE = '/profile-change';
  static const ORDER = '/order';
}

abstract class _Splash {
  _Splash._();
  static const SPLASH = '/splash';
}
